...
def test_complaint_preprocessor_integration():
    ...
    preprocessor = ComplaintPreprocessor(
        max_features=10
    )  # Using a small max_features for a predictable test
    preprocessor.fit(df)
    ...
    assert (
        X_transformed.shape[1] <= 10
    ), "Number of features should be at most max_features"
    ...
    assert (
        y_transformed.tolist() == expected_labels
    ), "Category consolidation should be applied"
    ...
    assert (
        len(preprocessor.vectorizer.vocabulary_) > 0
    ), "Vectorizer vocabulary should not be empty"
