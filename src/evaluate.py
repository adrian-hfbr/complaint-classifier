...
def run_evaluation(data_path: str, pipeline_path: str):
    ...
    with mlflow.start_run():
        ...
        df_raw = pd.read_csv(data_path, nrows=150000, low_memory=False)
        ...
        _, df_test = train_test_split(
            ...
            ],  # Stratify on the cleaned labels
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path", type=str, required=True, help="Path to the raw data file."
    )
    parser.add_argument(
        "--pipeline-path",
        type=str,
        required=True,
        help="Path to the best_pipeline.pkl file.",
    )
    args = parser.parse_args()

    run_evaluation(data_path=args.data_path, pipeline_path=args.pipeline_path)
